...
async fn run_agent_job(
    ...
    let memory_session_id = if matches!(job.session_target, SessionTarget::Main) {
        job.delivery
            .channel
            .as_deref()
            .filter(|channel| channel.eq_ignore_ascii_case("yuanqi"))
            .and(job.delivery.to.as_deref())
            .map(str::trim)
...
pub(crate) async fn deliver_announcement(
    ...
    match channel.to_ascii_lowercase().as_str() {
        ...
        "yuanqi" => {
            crate::gateway::yuanqi::publish_message(
                &config.workspace_dir,
                target,
                "assistant",
                output,
