            ...
            "properties": {
                ...
                "delivery": {
                    ...
                    "properties": {
                        ...
                        "channel": {
                            "type": "string",
                            "enum": ["telegram", "discord", "slack", "mattermost", "matrix", "yuanqi"],
...
mod tests {
    ...
    #[tokio::test]
    async fn delivery_schema_includes_matrix_and_yuanqi_channels() {
        let tmp = TempDir::new().unwrap();
        let cfg = test_config(&tmp).await;
        let tool = CronAddTool::new(cfg.clone(), test_security(&cfg));
        ...
        assert!(values.iter().any(|value| value == "yuanqi"));
    }
